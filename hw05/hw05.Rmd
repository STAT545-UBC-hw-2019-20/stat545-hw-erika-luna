...
The 'here' function from the 'here' package is very useful to do a couple of things faster, such as navigating in our project's directory. 

'here' is also a useful function when shraing code, because it doesn't matter the operating system that our collabotators use, this function runs independently. 
...
gapminder_ame_csv %>%
  knitr::kable()
```
However, notice that the variable country has now become a character: 
```{r}
...
To make it a factor again we do the following: 
```{r}
gapminder_ame_csv %>% 
